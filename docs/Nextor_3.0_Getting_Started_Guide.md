...
**Note:** Drive D: is the _ghost drive_ of the floppy disk drive, and it isn't created if you boot with the CTRL key pressed. You can also invert the meaning of that key, so that the ghost drive is created only when you _do_ press CTRL: run `CALL BOOTKEYS(&H2000)` in BASIC (and `CALL BOOTKEYS(-1)` to undo it). The setting is kept in the persistent storage of the Nextor controller, which for the Sunrise IDE driver is a file in the first partition of the first device, so it will work only once you have created and formatted the partitions in the next steps. Better not do it while following this guide, though: the drive letters in the examples that follow assume that you haven't. See _["Boot key inverters"](Nextor_3.0_User_Manual.md#2101-boot-key-inverters)_ and _["The CALL BOOTKEYS command"](Nextor_3.0_User_Manual.md#3616-the-call-bootkeys-command)_ in the Nextor 3.0 User Manual.
