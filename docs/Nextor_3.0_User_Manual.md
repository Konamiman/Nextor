...
Also, `NEXTOR.SYS` now looks for `AUTOEXEC.BTM` in the boot drive before `AUTOEXEC.BAT`: when a file with that name exists and can be opened for reading, it is the one executed at boot time (and `AUTOEXEC.BAT` is ignored, even if it exists too; if the `.BTM` cannot be opened, for example because of a disk error, `AUTOEXEC.BAT` is used as if the `.BTM` did not exist). The command interpreter loads `.BTM` batch files whole into memory before executing them, which is what allows the `GOTO`, `GOSUB`, `RETURN` and `END` commands in them (see the `BATCH` subject of the `HELP` command); thus the boot batch file can now use them as well. `AUTOEXEC.BTM` is looked for only when `NEXTOR.SYS` loads `COMMAND3.COM`: when it falls back to `COMMAND2.COM` (see _[3.10.1. How it is loaded](#3101-how-it-is-loaded)_), `AUTOEXEC.BAT` is executed as always, even if `AUTOEXEC.BTM` exists. As with `AUTOEXEC.BAT`, the boot drive is passed to the batch file as `%1`.
...
* The `INKEY` command accepts an optional flags number after the environment item name (`INKEY [text] %%name [flags]`): `1` makes it not wait for a key (if a keystroke is already waiting in the keyboard buffer it is read as usual, otherwise the environment item is deleted and the batch file continues at once, which allows a `.BTM` file to check for a key press while it repeats some commands), `2` makes it discard any keystrokes remaining in the keyboard buffer after reading the key (for example the repeats of a key held down), and `4` stores the code of the key as two hexadecimal digits instead of the key itself (`20` for the space bar, `1B` for ESC, `09` for TAB, `08` for BS and so on), without displaying the key, so that such keys can be tested with `IF`. The values can be added together (`7` does all three things). The default, `0`, is the usual behavior: the keyboard buffer is emptied and then the command waits for a key.
