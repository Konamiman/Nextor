; @@NAME@@ — Nextor disk driver (scaffolded by nextor-init).
;
; Starting point: the standalone dummy driver (a no-op driver that lets the
; kernel boot as a cartridge with no hardware). Replace the handler bodies
; below with your device logic. See the Driver Development Guide in the
; Nextor docs.
	.z80
	INCLUDE asm/macros/undoc.inc	;undoc-safe ld_/cp_/... macros (Z180)
...
;-----------------------------------------------------------------------------
	...
	; mknexrom expects the driver file to start with 256 dummy bytes (the
	; page-0 area), with the signature/jump table at 4100h. (When a driver
	; is instead linked into the kernel as a bank, the doshead fills 4000h-
	; 40FFh and this lead-in is omitted; here we build standalone, so we
