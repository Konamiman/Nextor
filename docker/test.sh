...
t "zip on PATH"                'command -v zip'

# --- baked content --------------------------------------------------------
t "NEXTOR_BASE file exists"    'test -f "$NEXTOR_BASE"'
t "SDK present"                'test -d "$NEXTOR_SDK/asm" && test -d "$NEXTOR_SDK/C"'
t "2 kernel base variants"     'test "$(ls /opt/nextor/kernel_base/kernel_base*.dat | wc -l)" -eq 2 && test -f /opt/nextor/kernel_base/kernel_base.dat && test -f /opt/nextor/kernel_base/kernel_base.NO_UNDOC.dat'
