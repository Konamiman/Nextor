/*
 * Partition and FAT filesystem structures and constants.
 */
...
#define PARTYPE_EXTENDED_LBA 15
...
/*
 * On-disk FAT boot sector layout (sector 0 of a FAT partition).
 */
typedef struct {
...
/*
 * On-disk MBR partition-table entry layout (16 bytes).
 *
 * Embedded inside masterBootRecord below.
 */
typedef struct {
	byte status;
...
/*
 * On-disk Master Boot Record layout (sector 0 of a partitioned device).
 *
 * Not returned by any DOS call; can be read or written via 
 * raw absolute-sector DOS access:
 *   _RDABS (2Fh) / _WRABS (30h) - via a logical drive letter
 *   _RDDRV (73h) / _WRDRV (74h) - via a driver slot/device/LUN
 *
 * For just *querying* an existing partition entry by index without
 * reading the MBR yourself, use _GPART (7Ah).
 */
typedef struct {
    byte jumpInstruction[3];
	char oemNameString[8];
